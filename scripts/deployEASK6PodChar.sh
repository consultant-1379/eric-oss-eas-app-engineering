...
helm install eric-oss-eas-app-engineering testware-repository/eric-oss-eas-app-engineering -n ${NAMESPACE} --kubeconfig ${KUBECONFIG} --set env.BUILD_URL=${BUILD_URL} --set env.APP_VERSION=${APP_VERSION} --set env.GAS_HOST_FROM_DEPLOY_SH=${GAS_HOST} --set env.IAM_HOST=${IAM_HOST} --set env.OPTIONS_FILE=${OPTIONS_FILE} --set env.MAIN_TEST_FILE="characteristicsTests.js" --set env.ENM_HOSTNAME=${ENM_HOSTNAME} --set env.CHAR_TEST_TYPE=${CHAR_TEST_TYPE} --set env.USE_SEF=${USE_SEF}